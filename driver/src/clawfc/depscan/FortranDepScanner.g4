...
USE_STMT : SEP? USE (SEP? ',' SEP? MODULE_NATURE)? (SEP | (SEP? '::' SEP?)) IDENTIFIER SEP? (',' SEP? (ONLY SEP? ':' SEP?)? RENAME_LIST_STMT)?;
...
fragment RENAME_STMT : USE_SYMBOL_NAME SEP? ('=>' SEP? USE_SYMBOL_NAME SEP?)?;

fragment MODULE_NATURE : INTRINSIC | NON_INTRINSIC;
fragment USE_SYMBOL_NAME : IDENTIFIER | OPERATOR_NAME;
fragment OPERATOR_NAME : OPERATOR SEP? '(' SEP? (~[)\r\n])+ SEP? ')' | ASSIGNMENT SEP? '(' SEP? '=' SEP? ')';
