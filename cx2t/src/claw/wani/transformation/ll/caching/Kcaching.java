 ...
 * transformation consists of placing an assignment in a scalar variable and use
 * this variable in a loop body before updating it.
 *
 * @author clementval
 */
public class Kcaching extends ClawTransformation
{
    ...
    public boolean analyze(XcodeProgram xcodeml, Translator translator)
    {
        ...
        if (_doStmt == null)
        {
            xcodeml.addError("The kcache directive is not nested in a do statement", _claw.getPragma().lineNo());
            return false;
        }
     ...
     */
    private void transformData(XcodeProgram xcodeml, FfunctionDefinition fctDef, String data, Translator translator)
            throws Exception
    {
        ...
        Xnode cacheVar = generateCacheVarAndAssignStmt(xcodeml, data, type, fctDef, aRefs.get(0), null);

        updateArrayRefWithCache(aRefs, cacheVar);

        if (_claw.hasClause(ClawClause.PRIVATE))
        {
            Directive.generatePrivateClause(xcodeml, _claw.getPragma(), cacheVar.value());
        }
    }
     ...
     * @throws Exception If something prevent the transformation to be done.
     */
    private void transformAssignStmt(XcodeProgram xcodeml, FfunctionDefinition fctDef, String data, Xnode stmt,
            Translator translator) throws Exception
    {
        String type = stmt.matchDirectDescendant(Xcode.F_ARRAY_REF).getType();
        List<Xnode> aRefs = checkOffsetAndGetArrayRefs(xcodeml, fctDef, data);

        Xnode cacheVar = generateCacheVarAndAssignStmt(xcodeml, data, type, fctDef, stmt, stmt);
    ...
    private void applyInitClause(XcodeProgram xcodeml, Translator translator, Xnode cacheVar, Xnode arrayRef)
    {
        ...
        if (_claw.hasClause(ClawClause.INIT))
        {
            ...
            if (initIfStmt == null)
            {
                ...
                // Set rhs of equality
                logEq.append(_doStmt.matchDirectDescendant(Xcode.INDEX_RANGE).matchDirectDescendant(Xcode.LOWER_BOUND)
                        .child(0), true);
     ...
     */
    private String generateNameWithOffsetInfo(String basename, List<Integer> offsets)
    {
     ...
     */
    private List<Integer> generateInferredOffsets(XcodeProgram xcodeml, FfunctionDefinition fctDef, String var)
            throws IllegalTransformationException
    {
        ...
        if (id == null)
        {
            throw new IllegalTransformationException(
                    "Variable " + var + " defined in the data clause has not been found", _claw.getPragma().lineNo());
        }
     ...
     */
    private Xnode generateCacheVarAndAssignStmt(XcodeProgram xcodeml, String var, String type,
            FfunctionDefinition fctDef, Xnode rhs, Xnode stmt)
    {
    ...
    private List<Xnode> checkOffsetAndGetArrayRefs(XcodeProgram xcodeml, FfunctionDefinition fctDef, String var)
            throws IllegalTransformationException
    {
        ...
        List<Xnode> arrayRefs = XnodeUtil.getAllArrayReferencesByOffsets(_doStmt.body(), var, offsets);
