 ...
 * information, they can be merge together. The transformation consists of
 * merging the body of the second do statement to the end of the first do
 * statement.
 *
 * @author clementval
 */
...
public class LoopFusion extends ClawTransformation
{
     ...
     *
     * @param directive The directive that triggered the loop fusion transformation.
     */
            ...
            throws IllegalTransformationException
    {
        ...
        if (_claw != null && _claw.hasClause(ClawClause.COLLAPSE) && _claw.getCollapseValue() > 0
                && _claw.getCollapseValue() > _doStmt.size())
        {
            throw new IllegalTransformationException("Cannot apply transformation, one or both do stmt are invalid.",
     ...
     * unit. To be able to be transformed together, the loop fusion units must share
     * the same parent block, the same iteration range, the same group option and
     * both units must be not transformed.
     *
     * @param transformation The other loop fusion unit to be merge with this one.
    ...
    public boolean canBeTransformedWith(XcodeProgram xcodeml, Transformation transformation)
    {
        ...
        if (!_doStmt.getOuterStatement().hasSameParentBlock(other.getNestedDoStmt().getOuterStatement()))
        {
            return false;
        }
        ...
        if (_claw.hasClause(ClawClause.COLLAPSE) && _claw.getCollapseValue() > 0)
        {
            ...
            {
                if (!Loop.hasSameIndexRange(_doStmt.get(i), other.getNestedDoStmt().get(i)))
                {
                    return false;
                }
            }
        ...
        } else
        {
            ...
            return Loop.hasSameIndexRange(_doStmt.getOuterStatement(), other.getNestedDoStmt().getOuterStatement());
        }
    }
     ...
     */
    private boolean hasCompatibleConstraints(XcodeProgram xcodeml, LoopFusion other)
    {
        ClawConstraint currentConstraint = ClawConstraint.DIRECT;
        if (_claw.hasClause(ClawClause.CONSTRAINT) && other.getLanguageInfo().hasClause(ClawClause.CONSTRAINT))
        {
        ...
        if (currentConstraint == ClawConstraint.DIRECT
                && !_doStmt.getOuterStatement().isDirectSibling(other.getNestedDoStmt().getOuterStatement(),
        ...
        } else
        {
            xcodeml.addWarning("Unconstrained loop-fusion generated",
                    Arrays.asList(_claw.getPragma().lineNo(), other.getLanguageInfo().getPragma().lineNo()));
        }
        return true;
    }

    /**
     * Return the do statement associated with this loop fusion unit at given depth.
     *
     ...
     * @return True if the two loop fusion units share the same group fusion option.
     */
    private boolean hasSameGroupClause(LoopFusion otherLoopUnit)
    {
        return (otherLoopUnit.getGroupClauseLabel() == null ? getGroupClauseLabel() == null
