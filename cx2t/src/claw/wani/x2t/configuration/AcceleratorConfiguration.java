...
public class AcceleratorConfiguration {
  ...
  private static final String ACCELERATOR_DATA_STRATEGY =
      "accelerator_data_strategy";
  private static final String ACCELERATOR_LOCAL_STRATEGY =
      "accelerator_local_strategy";
  private static final String ACCELERATOR_COLLAPSE = "accelerator_collapse";

  private AcceleratorDataStrategy _dataStrategy =
      AcceleratorDataStrategy.PRESENT;
  private AcceleratorLocalStrategy _localStrategy =
      AcceleratorLocalStrategy.PRIVATE;
  ...
  public AcceleratorConfiguration(Map<String, String> parameters) {
    if(parameters.containsKey(ACCELERATOR_DATA_STRATEGY)) {
      _dataStrategy = AcceleratorDataStrategy.
          fromString(parameters.get(ACCELERATOR_DATA_STRATEGY));
    }
    if(parameters.containsKey(ACCELERATOR_LOCAL_STRATEGY)) {
      _localStrategy = AcceleratorLocalStrategy.
          fromString(parameters.get(ACCELERATOR_LOCAL_STRATEGY));
    }
    if(parameters.containsKey(ACCELERATOR_COLLAPSE)) {
      _collapseStrategy =
          Boolean.parseBoolean(parameters.get(ACCELERATOR_COLLAPSE));
    }
  }
   ...
   */
  public AcceleratorDataStrategy getDataStrategy() { return _dataStrategy; }
   ...
   */
  public AcceleratorLocalStrategy getLocalStrategy() { return _localStrategy; }
