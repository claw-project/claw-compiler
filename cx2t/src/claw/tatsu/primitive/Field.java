...
/**
 * Primitive transformation and test applied to fields. This included: - Scalar
 * and array promotion (promote). - Adaptation of scalar reference to array
 * reference for promoted scalar. - Adaptation of allocate statement for
 * promoted field. - Demote array reference to fewer dimension or scalar.
 *
 * @author clementval
 */
public final class Field
{
     ...
     *                                        in typeTable.
     */
    public static void promote(PromotionInfo fieldInfo, FfunctionDefinition fctDef, XcodeProgram xcodeml)
            throws IllegalTransformationException
    {
        ...
        if (!FortranType.isBuiltInType(id.getType()) && crtType == null)
        {
            throw new IllegalTransformationException(
                    "Basic type of field " + fieldInfo.getIdentifier() + " could not be found");
        }

        FfunctionType fctType = xcodeml.getTypeTable().getFunctionType(fctDef);
        if (fctType == null)
        {
            throw new IllegalTransformationException("Function type " + fctDef.getType() + " could not be found",
                    fctDef.lineNo());
        }
        ...
        {
            throw new IllegalTransformationException(
                    "Promotion information has not " + "enough information. Dimension empty!", decl.lineNo());
        }
        ...
        if (crtType != null && crtType.isArray())
        {
            ...
            } else
            {
                ...
                if (old == null)
                {
                    throw new IllegalTransformationException("Cannot find type for " + fieldInfo.getIdentifier(),
                            decl.lineNo());
                } else
                {
        ...
        if (fieldInfo.getPromotionType() == PromotionInfo.PromotionType.ARRAY_TO_ARRAY)
        {
            if (newType.isAllAssumedShape()
                    && (fctType.hasParam(fieldInfo.getIdentifier()) || newType.isAllocatable() || newType.isPointer()))
            {
        ...
        if (fctType.hasAttribute(Xattr.RESULT_NAME)
                && fctType.getAttribute(Xattr.RESULT_NAME).equals(fieldInfo.getIdentifier()))
        {
     ...
     *                      be created.
     */
    public static void adaptScalarRefToArrayRef(PromotionInfo promotionInfo, FfunctionDefinition fctDef,
            List<DimensionDefinition> dims, XcodeML xcodeml)
    {
     ...
     */
    private static void demoteToScalar(Xnode arrayRef) throws IllegalTransformationException
    {
     ...
     * @throws IllegalTransformationException If node is null or not an FarrayRef.
     */
    private static void demote(Xnode arrayRef, List<Integer> keptDimensions) throws IllegalTransformationException
    {
        if (!Xnode.isOfCode(arrayRef, Xcode.F_ARRAY_REF))
        {
            throw new IllegalTransformationException(TatsuConstant.ERROR_INCOMPATIBLE);
     ...
     */
    public static void reshape(FfunctionDefinition fctDef, ReshapeInfo reshapeInfo, XcodeProgram xcodeml)
            throws IllegalTransformationException
    {
        ...
        {
            throw new IllegalTransformationException("Cannot apply reshape clause." + "Variable "
                    + reshapeInfo.getArrayName() + " not found in " + "declaration table.");
        }

        if (!(xcodeml.getTypeTable().isBasicType(id)))
        {
            throw new IllegalTransformationException(
                    String.format("Reshape variable %s is not a basic type.", reshapeInfo.getArrayName()));
        }
        FbasicType crtType = xcodeml.getTypeTable().getBasicType(id);
        ...
        {
            throw new IllegalTransformationException(String
                    .format("Reshape variable %s has smaller dimension than requested.", reshapeInfo.getArrayName()));
        }
        ...
        { // Demote to smaller dimension array

            if (crtType.getDimensions() - reshapeInfo.getKeptDimensions().size() != reshapeInfo.getTargetDimension())
            {
                throw new IllegalTransformationException(String.format(
                        "Reshape information for %s not valid. " + "Target dimension and kept dimension mismatch.",
     ...
     */
    private static Xnode convertVarToArrayRef(PromotionInfo promotionInfo, Xnode ref, XcodeML xcodeml)
    {
     ...
     */
    public static void adaptPointer(XcodeProgram xcodeml, FfunctionDefinition fctDef,
            Map<String, PromotionInfo> promotions, PromotionInfo pointeeInfo) throws IllegalTransformationException
    {
