 ...
 * - Extract directive prefix of a FpragmaStatement. - Split long pragma into
 * continued pragma. - Remove comment from a pragma - Find pragma into previous
 * siblings.
 *
 * @author clementval
 */
public final class Pragma
{
    ...
    {
        if (!Xnode.isOfCode(pragma, Xcode.F_PRAGMA_STATEMENT) || pragma.value().isEmpty())
        {
            return "";
        }
    ...
    /**
     * Split a long pragma into chunk depending on the maxColumns specified. The
     * split is done on spaces or commas.
     *
     * @param fullPragma   The original full pragma value.
     * @param maxColumns   Maximum number of columns. This length take into account
     *                     the added prefix and continuation symbol.
     * @param pragmaPrefix Prefix used by the pragma.
     * @return A list of chunks from the original pragma.
     */
    public static List<String> split(String fullPragma, int maxColumns, String pragmaPrefix)
    {
        ...
        if (fullPragma.length() > maxColumns)
        {
            ...
            {
                int splitIndex = fullPragma.substring(0, maxColumns - addLength).lastIndexOf(' ');
                // Cannot cut as it should. Take first possible cutting point.
                if (splitIndex == -1)
                {
                    splitIndex = fullPragma.substring(0, maxColumns - addLength).lastIndexOf(',');
                    if (splitIndex == -1)
                    {
                        splitIndex = (fullPragma.contains(" ")) ? fullPragma.lastIndexOf(' ')
                                : (fullPragma.contains(",")) ? fullPragma.lastIndexOf(',') : fullPragma.length();
                    }
                }
     ...
     *                                        FpragmaStatement.
     */
    public static void splitByLength(Xnode pragma, XcodeProgram xcodeml, String prefix)
            throws IllegalTransformationException
    {
        ...
        if (allPragma.length() > xcodeml.context().getMaxColumns())
        {
            ...
            for (int i = 0; i < splittedPragmas.size(); ++i)
            {
                ...
                newlyInserted = createAndInsertPragma(xcodeml, newlyInserted, pragma.filename(), pragma.lineNo(),
     ...
     *                                        FpragmaStatement.
     */
    public static void splitByCont(Xnode pragma, String prefix, XcodeProgram xcodeml)
            throws IllegalTransformationException
    {
        if (!Xnode.isOfCode(pragma, Xcode.F_PRAGMA_STATEMENT))
        {
        ...
        for (int i = 0; i < lines.length - 1; ++i)
        {
            if (!lines[i].isEmpty())
            {
                newlyInserted = createAndInsertPragma(xcodeml, newlyInserted, pragma.filename(), lineIndex, lines[i],
                        true);
                ++lineIndex;
            }
        }
    ...
    public static CompilerDirective getCompilerDirective(Xnode pragma)
    {
        ...
        } else if (pragma.value().toLowerCase().contains(TatsuConstant.CLAW_PREFIX))
        {
     ...
     */
    private static Xnode createAndInsertPragma(XcodeProgram xcodeml, Xnode hook, String filename, int lineNo,
            String value, boolean continued)
    {
     ...
     *
     * @param directive Compiler directive to delete. NONE means both
     *                  OpenACC/OpenMP.
    ...
    public static void remove(CompilerDirective directive, Xnode start, Xnode end)
    {
        ...
        List<Xnode> pragmas = XnodeUtil.getNodes(start, end, Collections.singletonList(Xcode.F_PRAGMA_STATEMENT));

        for (Xnode p : pragmas)
        {
            CompilerDirective pDir = getCompilerDirective(p);
            if (directive == pDir || (directive == CompilerDirective.NONE
                    && (pDir == CompilerDirective.OPENACC || pDir == CompilerDirective.OPENMP)))
            {
                p.delete();
            }
        }
