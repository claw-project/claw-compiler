...
public class NestedDoStatement
{
    ...
    public NestedDoStatement(Xnode outerDoStatement, int nb)
    {
        ...
        while (crtDoStatement != null)
        {
            ...
            crtDoStatement = crtDoStatement.body().matchDirectDescendant(Xcode.F_DO_STATEMENT);
     ...
     * The outer statement represents the first element of the list and the inner
     * statement represents the last element of the list.
     *
     * @param dimensions A list of dimension objects.
     * @param xcodeml    The current XcodeML translation unit in which the elements
     *                   will be created.
     */
    public NestedDoStatement(List<DimensionDefinition> dimensions, XcodeProgram xcodeml)
    {
        _statements = new ArrayList<>();
        for (DimensionDefinition dim : dimensions)
        {
            Xnode induction = xcodeml.createVar(FortranType.INTEGER, dim.getIdentifier(), Xscope.LOCAL);
     ...
     *                will be created.
     */
    public NestedDoStatement(List<DimensionDefinition> dimensions, PromotionInfo pi, XcodeProgram xcodeml)
    {
        _statements = new ArrayList<>();
        int index = 1;
        for (DimensionDefinition dim : dimensions)
        {
            _statements.add(Loop.createDoStmtOverAssumedShapeArray(pi.getTargetType(), pi.getIdentifier(),
                    dim.getIdentifier(), index, xcodeml));
            ++index;
        }
    }
     ...
     * position of the xth index from right to left. E.g. current to new position:
     * i,j,k &rarr; k,i,j = 120 i goes to pos 1, j goes to pos 2 and k goes to pos
     * 0.
     *
