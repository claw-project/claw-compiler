...
public class DimensionDefinition
{
    ...
    public static final DimensionDefinition BASE_DIMENSION = createBaseDimension();
    ...
    /**
     * Create the special dimension definition used as place holder for currently
     * existing dimensions.
     *
    ...
    {
        return new DimensionDefinition(BASE_DIMENSION_ID, new BoundDefinition("", BoundDefinition.BoundType.LOWER),
     ...
     */
    private DimensionDefinition(String id, BoundDefinition lowerBound, BoundDefinition upperBound)
    {
        _identifier = id;
        ...
        _iterationStep = new BoundDefinition(String.valueOf(DEFAULT_STEP_VALUE), BoundDefinition.BoundType.STEP);
    }
    ...
    {
        _identifier = id;
        _lowerBound = new BoundDefinition(lowerBound, BoundDefinition.BoundType.LOWER);
        _upperBound = new BoundDefinition(upperBound, BoundDefinition.BoundType.UPPER);
     ...
     */
    public DimensionDefinition(String id, String lowerBound, String upperBound, String itLowerBound,
            String itUpperBound, String step)
    {
        _identifier = id;
        _lowerBound = new BoundDefinition(lowerBound, BoundDefinition.BoundType.LOWER);
        _upperBound = new BoundDefinition(upperBound, BoundDefinition.BoundType.UPPER);
        ...
        {
            _iterationLowerBound = new BoundDefinition(itLowerBound, BoundDefinition.BoundType.LOWER);
        } else
        {
            _iterationLowerBound = _lowerBound;
        }
        ...
        {
            _iterationUpperBound = new BoundDefinition(itUpperBound, BoundDefinition.BoundType.UPPER);
        } else
        {
            _iterationUpperBound = _upperBound;
        }
        ...
        if (step != null)
        {
            _iterationStep = new BoundDefinition(step, BoundDefinition.BoundType.STEP);
        } else
        {
            _iterationStep = new BoundDefinition(String.valueOf(DEFAULT_STEP_VALUE), BoundDefinition.BoundType.STEP);
        }
    ...
    /**
     * Generate the correct indexRange element with lowerBound, upperBound and step
     ...
     * @param useIteration IF true, use the iterations bounds instead of the size
     *                     bounds.
    ...
    /**
     * Generate the array index that will be placed in the array reference for this
    ...
    public Xnode generateArrayIndex(XcodeML xcodeml)
    {
        ...
        aIdx.append(xcodeml.createVar(FortranType.INTEGER, _identifier, Xscope.LOCAL));
        return aIdx;
    }

    /**
     * Generate additional node to be inserted in the allocate statement arrayIndex
     * based on the dimension definition.
     *
     * @param xcodeml Current XcodeML program unit.
    ...
    public String toString()
    {
        return String.format("%s(%s:%s)", _identifier,
                _lowerBound.isVar() ? _lowerBound.getValue() : String.valueOf(_lowerBound.getIntValue()),
                _upperBound.isVar() ? _upperBound.getValue() : String.valueOf(_upperBound.getIntValue()));
    }
    ...
    {
        int baseDimensionNb = Collections.frequency(dimensions, DimensionDefinition.BASE_DIMENSION);
