...
public class Xnode
{
    ...
    {
        return hasAttribute(attrCode) && _baseElement.getAttribute(attrCode).equals(Xname.TRUE);
    }
    ...
    public String value()
    {
        return _baseElement == null ? "" : _baseElement.getTextContent().trim().toLowerCase();
    }
     ...
     * @param clone If true, the element is cloned before being appended. If false,
     *              the element is directly appended.
     * @return Current object to allow chaining.
     */
    ...
    /**
     * Check whether the end node is a direct sibling of this node. If other nodes
     * are between the two nodes and their opcode is not listed in the skippedNodes
     * list, the nodes are not direct siblings.
     *
    ...
    public Xnode matchDescendant(Xcode opcode)
    {
        ...
        return (elements.item(0) == null) ? null : new Xnode((Element) elements.item(0));
    }
    ...
    /**
     * Find node with one of the given opcodes in the direct descendants the current
     * node.
     *
    ...
    private Xnode universalMatch(Xcode opcode, boolean down)
    {
        ...
        Node nextNode = down ? _baseElement.getNextSibling() : _baseElement.getParentNode();
    ...
    {
        return opcode() == Xcode.F_REAL_CONSTANT || opcode() == Xcode.F_INT_CONSTANT
                || opcode() == Xcode.F_COMPLEX_CONSTANT || opcode() == Xcode.F_CHARACTER_CONSTANT
     ...
     */
    public String constructRepresentation(boolean withNamedValue, boolean nameOnly)
    {
        switch (opcode())
        {
        case F_INT_CONSTANT:
        ...
        case INDEX_RANGE:
            return nameOnly ? "" : constructIndexRangeRepresentation(withNamedValue, nameOnly);
        case F_ARRAY_REF:
     ...
     */
    private String constructBinaryExprRepresentation(boolean withNamedValue, boolean nameOnly)
    {
        Xnode child0 = child(0);
        Xnode child1 = child(1);
        return ((child0 != null) ? child0.constructRepresentation(withNamedValue, nameOnly) : "")
                + getBinaryOpRepresentation()
                + ((child1 != null) ? child1.constructRepresentation(withNamedValue, nameOnly) : "");
    }
     ...
     * @return String representation. Null if node is null.
     */
    private String constructIndexRangeRepresentation(boolean withNamedValue, boolean nameOnly)
    {
     ...
     * @return String representation. Null if node is null.
     */
    private String constructSimpleRepresentation(boolean withNamedValue, boolean nameOnly)
    {
        Xnode n = firstChild();
        return (n != null) ? n.constructRepresentation(withNamedValue, nameOnly) : "";
    }
     ...
     * @return String representation. Null if node is null.
     */
    private String constructArrayRefRepresentation(boolean withNamedValue, boolean nameOnly)
    {
        ...
        } else
        {
            ...
            str.append(childs.get(0).constructRepresentation(withNamedValue, nameOnly));
            if (nameOnly)
            {
                return str.toString();
            }
            ...
            {
                str.append(childs.get(i).constructRepresentation(withNamedValue, nameOnly));
                if (i != childs.size() - 1)
                {
                    str.append(",");
                }
     ...
     * @return String representation. Null if node is null.
     */
    private String constructMemberRefRepresentation(boolean withNamedValue, boolean nameOnly)
    {
        Xnode n = firstChild();
        return ((n != null) ? n.constructRepresentation(withNamedValue, nameOnly) + "%" + getAttribute(Xattr.MEMBER)
                : "");
    }
     ...
     * @return String representation. Null if node is null.
     */
    private String constructNamedValueRepresentation(boolean withNamedValue, boolean nameOnly)
    {
        Xnode n = firstChild();
        if (withNamedValue)
        {
            return ((n != null) ? getAttribute(Xattr.NAME) + "=" + n.constructRepresentation(true, nameOnly) : "");
        }
    ...
    /**
     * Sync given attribute between two node. Attribute present in this node is
     * created in "to" if not present yet. Attribute not present in this node is
     * removed from "to" if present.
     *
    ...
    public void syncBooleanAttribute(Xnode to, Xattr attribute)
    {
        ...
        } else if (!getBooleanAttribute(attribute) && to.getBooleanAttribute(attribute))
        {
    ...
    {
        return String.format("%s (children: %d) - %d", opcode().code(), children().size(), lineNo());
    }
     ...
     *
     * @return True if the node is not a direct child of an arrayIndex node. False
     *         otherwise.
     */
    ...
    {
        return !ancestor().ancestorIs(Xcode.F_ARRAY_REF) && !ancestorIs(Xcode.F_ARRAY_REF);
    }
