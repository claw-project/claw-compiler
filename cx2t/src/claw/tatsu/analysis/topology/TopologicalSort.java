 ...
 */
package claw.tatsu.analysis.topology;
...
public final class TopologicalSort
{
    ...
    /**
     * Given a directed acyclic graph, returns a topological sorting of the nodes in
     * the graph. If the input graph is not a DAG, throws an
    ...
    public static <T> List<T> sort(DirectedGraph<T> g)
    {
        ...
        /*
         * Maintain two structures - a set of visited nodes (so that once we've added a
         * node to the list, we don't label it again), and a list of nodes that actually
         * holds the topological ordering.
         */
        List<T> result = new ArrayList<>();
        Set<T> visited = new HashSet<>();

        /*
         * We'll also maintain a third set consisting of all nodes that have been fully
         * expanded. If the graph contains a cycle, then we can detect this by noting
         * that a node has been explored but not fully expanded.
         */
    ...
    private static <T> void explore(T node, DirectedGraph<T> g, List<T> ordering, Set<T> visited, Set<T> expanded)
    {
        ...
        if (visited.contains(node))
        {
             ...
             * expanded, then it's already been assigned a position in the final topological
             * sort and we don't need to explore it again. However, if it hasn't been
             * expanded, it means that we've just found a node that is currently being
             * explored, and therefore is part of a cycle. In that case, we should report an
             * error.
             */
        ...
        /*
         * Having explored all of the node's predecessors, we can now add this node to
         * the sorted ordering.
         */
        ordering.add(node);
